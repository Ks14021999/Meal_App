...
function removeFromfavorites(meal_id) {
    ...
    var arrfavorites = localStorage.setItem('arrfavorites','');
    let id=""
    for(let data of displayArr){
        id += "," + data ;
    }
    localStorage.setItem('arrfavorites', id.slice(1,));
                        ...
                        <div class="card" id = "${data.meals[0].idMeal}" style="width: 18rem; margin-bottom:3%; border-radius: 30px; overflow:hidden">
                            ...
                            <div class="card-body">
                                ...
                                <a href="detailsPage.html" 
                                    id = "show-more" onclick="return addMealIdToLocal(${data.meals[0].idMeal},'${data.meals[0].strMeal}')" 
                                    class="btn btn-success">See More Details
                                </a>
                                <button type = "submit" 
                                    onclick = "removeFromfavorites(${data.meals[0].idMeal})" 
                                    class="btn btn-danger" style="margin-top: 3%; ">Remove From Favorites
                                </button>
                            </div>
                        </div>
                    `;
